...
String topic_status;
String topic_restore;     // dulunya: upload  → sekarang: restore (terima template dari server)
String topic_command;
String topic_display;
String topic_message;     // [BARU] display custom dari server
String topic_sensorinfo;  // [BARU] publish sensor id ke server
String topic_slots;       // [BARU] publish list slot terisi ke server
...
void updateTopics() {
  ...
  topic_message     = "absensi/" + sid + "/message";     // [BARU]
  topic_sensorinfo  = "absensi/" + sid + "/sensorinfo";  // [BARU]
  topic_slots       = "absensi/" + sid + "/slots";       // [BARU]
}
...
// ====================== SETUP WIFI + MQTT VIA WIFIMANAGER ======================
void setupWiFiAndConfig() {
  ...
  Serial.printf("MQTT: %s:%s user=%s\n", mqtt_server, mqtt_port_str, mqtt_user);
  Serial.printf("Sensor ID: %s\n", sensor_id);
}

// ====================== PUBLISH SENSOR INFO [BARU] ======================
// Dipanggil otomatis setiap kali connect ke MQTT
void publishSensorInfo() {
  ...
  Serial.println("Sensor info dipublish: " + payload);
}

// ====================== PUBLISH SLOT TERISI [BARU] ======================
// Dipanggil saat server kirim command get_slots
void publishSlots() {
  lcdPrint("Scanning Slots", "Please wait...");

  // Kumpulkan semua ID yang terisi
  String slotList = "[";
  bool first = true;
  ...
  // Buat payload JSON manual (hindari buffer overflow untuk list panjang)
  String payload = "{\"sensor_id\":\"" + String(sensor_id) + "\",\"slots\":" + slotList + "}";

  // Publish dengan beginPublish karena payload bisa besar
  ...
  Serial.println("Slots dipublish: " + payload);
  delay(1500);
...
void mqttReconnect() {
  ...
  if (mqttClient.connect(clientId.c_str(), mqtt_user, mqtt_pass)) {
    ...
    mqttClient.subscribe(topic_restore.c_str());   // dulunya: upload
    mqttClient.subscribe(topic_command.c_str());
    mqttClient.subscribe(topic_display.c_str());
    mqttClient.subscribe(topic_message.c_str());   // [BARU]

    lcdPrint("MQTT Connected", "ID: " + String(sensor_id));
    delay(1000);

    // [BARU] Auto publish sensor info saat pertama connect
    publishSensorInfo();
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // ------------------------------------------------
  // TOPIC: /display — hasil login dari server
  // ------------------------------------------------
  if (String(topic) == topic_display) {
    JsonDocument doc;
    if (!deserializeJson(doc, msg)) {
      if (doc["status"] == "sukses") {
        lcdPrint("Akses Diterima", doc["name"].as<String>());
        buzzerSukses();
      } else {
        lcdPrint("Akses Ditolak", doc["name"].as<String>());
        buzzerGagal();
      }
      delay(3000);
      if (isLoginMode) lcdPrint("Mode: LOGIN", "Tempelkan Jari");
  ...
  // ------------------------------------------------
  // TOPIC: /restore — terima template dari server (dulunya: /upload)
  // ------------------------------------------------
  ...
  // ------------------------------------------------
  // TOPIC: /message — [BARU] display custom dari server
  // Format: {"line1": "Hello", "line2": "World"}
  // ------------------------------------------------
    ...
    if (!deserializeJson(doc, msg)) {
      ...
      buzzerNotif();
      // Tidak kembali ke mode LOGIN otomatis — biarkan server yang kontrol
    }
  }

  // ------------------------------------------------
  // TOPIC: /command — semua perintah dari server (JSON)
  // ------------------------------------------------
  else if (String(topic) == topic_command) {
    JsonDocument doc;
    if (deserializeJson(doc, msg)) return; // JSON invalid, abaikan

    String command = doc["command"] | "";

    // --- register: daftarkan sidik jari baru ---
    if (command == "register") {
      int idBaru = cariIdKosong();
      if (idBaru != -1) {
        lcdPrint("Register ID:" + String(idBaru), "Tempelkan Jari");
        buzzerNotif();
    ...
    // --- upload_done: notifikasi semua template sudah dikirim server (dulunya: restore_done) ---
    else if (command == "upload_done") {
      lcdPrint("Upload Selesai!", "Semua Data Aman");
      buzzerSukses();
      mqttClient.publish(topic_status.c_str(),
        "{\"status\":\"sukses\",\"message\":\"all data restored\"}");
      delay(2000);
      if (isLoginMode) lcdPrint("Mode: LOGIN", "Tempelkan Jari");
    }

    // --- delete_all: hapus semua template ---
    ...
    // --- [BARU] delete: hapus template satuan ---
    // Format: {"command": "delete", "template_id": 5}
      ...
      if (targetId == -1) {
        ...
        if (finger.deleteModel(targetId) == FINGERPRINT_OK) {
          ...
          buzzerSukses();
          mqttClient.publish(topic_status.c_str(),
            ("{\"status\":\"sukses\",\"message\":\"deleted\",\"template_id\":" + String(targetId) + "}").c_str());
        } else {
    ...
    // --- [BARU] get_sensor_id: kirim sensor id ke server ---
    else if (command == "get_sensor_id") {
      publishSensorInfo();
    }

    // --- [BARU] get_slots: kirim semua slot terisi ke server ---
...
void cekSidikJariContinuous() {
  ...
  lcdPrint("Processing...", "Scanning...");
  p = finger.image2Tz();
  if (p != FINGERPRINT_OK) {
    lcdPrint("Coba Lagi", "Jari Kurang Pas");
    delay(1000);
...
void prosesRegister(int id) {
  ...
  if (!ok1) { lcdPrint("Timeout!", "Batal"); buzzerGagal(); delay(2000); if (isLoginMode) lcdPrint("Mode: LOGIN", "Tempelkan Jari"); return; }

  lcdPrint("Angkat Jari", "Tempelkan Lagi"); buzzerNotif(); delay(1500);
  ...
  if (!ok2) { lcdPrint("Timeout!", "Batal"); buzzerGagal(); delay(2000); if (isLoginMode) lcdPrint("Mode: LOGIN", "Tempelkan Jari"); return; }
  ...
  if (finger.createModel() == FINGERPRINT_OK && finger.storeModel(id) == FINGERPRINT_OK) {
    ...
    lcdPrint("Gagal!", "Jari Tdk Cocok"); buzzerGagal(); delay(2000);
  }
  if (isLoginMode) lcdPrint("Mode: LOGIN", "Tempelkan Jari");
}
...
void kirimTemplateViaMQTT(int id) {
  ...
  if (mqttClient.beginPublish(topic_register.c_str(), payload.length(), false)) {
    ...
    lcdPrint("Selesai!", "Data Terkirim"); buzzerSukses();
  } else {
    lcdPrint("Upload Gagal", "Buffer Penuh"); buzzerGagal();
  }
  delay(1500);
}
...
void setup() {
  ...
  mqttClient.setBufferSize(4096); // dinaikkan dari 2048 → 4096 untuk handle payload slots yang besar
...
void loop() {
  ...
  mqttReconnect();
  mqttClient.loop();
  if (isLoginMode && mqttClient.connected()) {
